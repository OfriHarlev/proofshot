...
<Callout type="info">
**Prerequisites:** [GitHub CLI](https://cli.github.com/) (`gh`) installed and authenticated. At least one completed ProofShot session on the current branch.
</Callout>
...
3. Screenshots and video are uploaded as GitHub user-attachments
4. If ffmpeg is available, `.webm` video is converted to `.mp4` for better browser compatibility
5. A formatted markdown comment is posted on the PR with inline media
...
GitHub doesn't support `.webm` in markdown comments. Install ffmpeg so ProofShot can convert the video to `.mp4` before uploading.
