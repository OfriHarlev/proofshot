...
describe('buildAgentBrowserCommand', () => {
  ...
  it('applies default config path options to later commands', () => {
    ...
    expect(buildAgentBrowserCommand('snapshot -i', { session: 'proofshot-dev' })).toBe(
      "agent-browser --config '/tmp/project-agent-browser.json' --session 'proofshot-dev' snapshot -i",
    );
  });
});
